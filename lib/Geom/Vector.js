 ...
 * It implemets some methods which
 * can be useful for physics calculations.
 * @memberof Geom
 * @extends Dot
...
class Vector extends Dot {
   ...
   */
  get angle() {
    return Math.atan(Math.abs(this.y / this.x));
  }
  ...
  set length(len) {
    ...
    this.x = len * Math.cos(angle) * Math.sign(this.x);
    this.y = len * Math.sin(angle) * Math.sign(this.y);
    return {
      x: this.x,
      y: this.y,
    };
